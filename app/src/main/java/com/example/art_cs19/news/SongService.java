...
public class SongService extends Service implements AudioManager.OnAudioFocusChangeListener{
	...
	private void newNotification() {
		...
		try{
			...
			if(albumArt != null){
				...
				notification.contentView.setImageViewResource(R.id.imageViewAlbumArt, R.drawable.logomusic);
				if(currentVersionSupportBigNotification){
					notification.bigContentView.setImageViewResource(R.id.imageViewAlbumArt, R.drawable.logomusic);
				}
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	private void UpdateMetadata(MediaItem data){
		...
		if(mDummyAlbumArt == null){
			mDummyAlbumArt = BitmapFactory.decodeResource(getResources(), R.drawable.logomusic);
		}
